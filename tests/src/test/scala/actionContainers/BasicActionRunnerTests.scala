...
trait BasicActionRunnerTests extends ActionProxyContainerTestUtils {
  ...
  /**
   * Configuration for runtime unit test.
   *
   * @param code the code as string (may be plain text or base64 encoded for binaries)
   * @param main the name of the main function (for native action this has no real effect)
   * @param enforceEmptyOutputStream if true, will confirm the stdout stream is empty modulo the frame marker
   * @param enforceEmptyErrorStream if true, will confirm the stderr stream is empty modulo the frame marker
   * @param hasCodeStub this is an escape hatch for native runtimes that might provide a stub executable
   * @param skipTest this is an escape hatch to bypass some tests which might not yet satisfy the invariants required
   */
