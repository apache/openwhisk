...
class ApiGwRestRoutemgmtActionTests
    extends TestHelpers
    ...
    def getApis(
        ...
        val rr = wsk.action.invoke(
            ...
            blocking = true,
            result = true,
            expectedExitCode = OK.intValue)(wskprops)
        var apiJsArray: JsArray =
            try {
                var apisobj = rr.respBody.fields("apis")
    ...
    def createApi(
        ...
        expectedExitCode: Int = OK.intValue): RestResult = {
        val parms = Map[String, JsValue]() ++
    ...
    it should "verify successful creation of a new API" in {
        ...
        try {
            ...
            JsObjectHelper(createResult.respBody).fieldPathExists("apidoc") should be(true)
    ...
    // CLI specific tests, no need to convert them into rest.
    ...
    it should "verify successful creation of a new V2 API" in {
        ...
        try {
            val createResult = createApiV2(namespace = Some(wskprops.namespace), basepath = Some(testbasepath), relpath = Some(testrelpath),
                operation = Some(testurlop), apiname = Some(testapiname), action = Some(testaction))
            println("createResultv2 is " + createResult)
            JsObjectHelper(createResult.respBody).fieldPathExists("apidoc") should be(true)
