...
class ContainerPoolTests
  ...
  it should "remove a container to make space in the pool if it is already full and a different action arrives" in within(
    ...
    val pool = system.actorOf(ContainerPool.props(factory, ContainerPoolConfig(1, 1, false), feed.ref))
    pool ! runMessage
    containers(0).expectMsg(runMessage)
    containers(0).send(pool, NeedWork(warmedData()))
  ...
  it should "cache a container if there is still space in the pool" in within(timeout) {
    ...
    val pool = system.actorOf(ContainerPool.props(factory, ContainerPoolConfig(1, 2, false), feed.ref))
  ...
  it should "create prewarmed containers on startup" in within(timeout) {
    ...
    val pool =
      system.actorOf(
        ContainerPool
          .props(factory, ContainerPoolConfig(0, 0, false), feed.ref, List(PrewarmingConfig(1, exec, memoryLimit))))
  ...
  it should "use a prewarmed container and create a new one to fill its place" in within(timeout) {
    ...
    val pool =
      system.actorOf(
        ContainerPool
          .props(factory, ContainerPoolConfig(1, 1, false), feed.ref, List(PrewarmingConfig(1, exec, memoryLimit))))
    containers(0).expectMsg(Start(exec, memoryLimit))
  ...
  it should "not use a prewarmed container if it doesn't fit memory wise" in within(timeout) {
    ...
    val pool =
      system.actorOf(ContainerPool
        .props(factory, ContainerPoolConfig(1, 1, false), feed.ref, List(PrewarmingConfig(1, exec, alternativeLimit))))
