...
class AuthenticateTests extends ControllerTestCommon with Authenticate {
    ...
    it should "authorize a known user using different namespaces and cache key, and reject invalid secret" in {
        ...
        namespaces.foreach { ns =>
            ...
            stream.toString should include(s"serving from datastore: ${ns.authkey.asCacheKey}")
            stream.reset()
            ...
            stream.toString should include(s"serving from cache: ${ns.authkey.asCacheKey}")
            stream.reset()
        }
