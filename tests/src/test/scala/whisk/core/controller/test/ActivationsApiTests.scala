...
class ActivationsApiTests extends ControllerTestCommon with WhiskActivationsApi {
    ...
    it should "report proper error when record is corrupted on get" in {
        ...
        val activationStore = SpiLoader.get[ArtifactStoreProvider]().makeStore[WhiskEntity, WhiskEntity](whiskConfig, _.dbActivations, None)(WhiskEntityJsonFormat, system, logging)
        implicit val tid = transid()
