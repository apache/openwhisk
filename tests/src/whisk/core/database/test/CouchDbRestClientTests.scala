...
class CouchDbRestClientTests extends FlatSpec
    ...
    // the (fake) DB name doesn't matter as this test only execises the "/" endpoint ("instance info")
    val client = CouchDbRestClient.make("http", "localhost", 15975, config.dbUsername, config.dbPassword, "fakeDB")
