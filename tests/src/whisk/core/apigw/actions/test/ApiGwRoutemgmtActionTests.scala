...
class ApiGwRoutemgmtActionTests
    ...
    val whiskkey = retry(Await.result(WhiskAuth.get(authstore, Subject("whisk.system"), false)(TransactionId.unknown), 5.seconds), N = 3, waitBeforeRetry = Some(1.second))
        ...
        val invalidArgs = Seq(
            ...
            ("/whisk.system/routemgmt/createApi", ANY_ERROR_EXIT, "action is required when swagger is not specified",
                Seq(
                    ...
                    "-p", "basepath", "/ApiGwRoutemgmtActionTests_bp",
                    "-p", "relpath", "/ApiGwRoutemgmtActionTests_rp",
                    "-p", "operation", "get" //"-p", "action", """{"backendUrl":"URL","backendMethod":"get","namespace":"_","name":"myaction","authkey":"XXXXX"}"""
                    )),
        ...
        invalidArgs foreach {
            case (action: String, exitcode: Int, errmsg: String, params: Seq[String]) =>
                var cmd: Seq[String] = Seq("action",
                    ...
                    "--auth", whisksystemAuthId + ":" + whisksystemAuthPwd) ++ params
