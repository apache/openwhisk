...
Feature:  This feature file will test all the wsk functions.It will use User3 credentails
  ...
  Background:
* configure ssl = true
* def nameSpace = NS_botTester7
* def Auth = Auth_botTester7
* def params = '?blocking=true&result=false'
* def scriptcode = call read('classpath:com/karate/openwhisk/functions/hello-world.js')



  Scenario: TC05_ConcurrentUser7-As a user I want to all the wsk functions available to the user and check if they give the proper response
