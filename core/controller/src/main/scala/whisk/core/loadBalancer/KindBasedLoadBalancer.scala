...
class KindBasedExecutorLoadBalancerProvider extends LoadBalancerProvider {
    override def getLoadBalancer(config: WhiskConfig, instance: InstanceId)
            (implicit logging: Logging, actorSystem: ActorSystem): LoadBalancer = new KindBasedLoadBalancer(config, instance)

}
object KindBasedExecutorLoadBalancerProvider extends SpiFactory[LoadBalancerProvider]{
    override def apply(dependencies: Dependencies): LoadBalancerProvider = new KindBasedExecutorLoadBalancerProvider
}
...
class KindBasedLoadBalancer(config: WhiskConfig,
    ...
    val executorForKind = TrieMap[String, Option[ActivationExecutor]]()
