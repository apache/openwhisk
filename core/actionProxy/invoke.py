...
DEST = os.environ.get("DOCKER_HOST")
if DEST:
    try:
        DOCKER_HOST = re.compile("tcp://(.*):[\d]+").findall(DEST)[0]
    except Exception:
        print("cannot determine docker host from %s" % DEST)
        sys.exit(-1)
...
def run(args):
    value = content_from_args(args) or {}
    if not value:
