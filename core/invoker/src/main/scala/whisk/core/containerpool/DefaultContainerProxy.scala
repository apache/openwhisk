  ...
  when(Uninitialized) {
    ...
    case Event(job: Start, _) =>
      factory(
        ...
        DefaultContainerProxy.containerName(instance, "prewarm", job.exec.kind),
        job.exec.image,
        job.exec.pull,
        job.memoryLimit,
        poolConfig.cpuShare)
    ...
    case Event(job: Run, _) =>
      ...
      val container = factory(
        job.msg.transid,
        DefaultContainerProxy.containerName(instance, job.msg.user.namespace.name, job.action.name.name),
      ...
      container
        .andThen {
          ...
          case Failure(t) =>
            ...
            val activation = DefaultContainerProxy.constructWhiskActivation(job, None, Interval.zero, response)
      ...
      .flatMap { initInterval =>
        ...
        container.run(parameters, environment, actionTimeout)(job.msg.transid).map {
          case (runInterval, response) =>
            ...
            DefaultContainerProxy.constructWhiskActivation(job, initInterval, initRunInterval, response)
        }
      }
      .recover {
        case InitializationError(interval, response) =>
          DefaultContainerProxy.constructWhiskActivation(job, Some(interval), interval, response)
        case t =>
...
object DefaultContainerProxy {
  def props(
    ...
    Props(new DefaultContainerProxy(factory, ack, store, collectLogs, instance, poolConfig, unusedTimeout, pauseGrace))
