...
object DefaultContainerPool extends ContainerPoolProvider {
  ...
  override def getContainerProxyFactory(
    ...
    instance: InstanceId,
    poolConfig: ContainerPoolConfig): ActorRefFactory => ActorRef = { f: ActorRefFactory =>
    f.actorOf(DefaultContainerProxy.props(factory, ack, store, collectLogs, instance, poolConfig))
  }
