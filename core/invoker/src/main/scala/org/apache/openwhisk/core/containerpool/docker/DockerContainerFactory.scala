...
/**
 * This contains factory provides a Docker for Mac client which exposes action container's ports on the host.
 */
    ...
    super
      .runCmd(
        Seq(
          "inspect",
          "--format",
          s"""{{(index (index .NetworkSettings.Ports "8080/tcp") 0).HostPort}}""",
          id.asString),
