 ...
 */
class CloudantStore[RawDocument, DocumentAbstraction](
    dbUsername: String,
    dbPassword: String,
    dbName: String)(implicit serializerEvidence: DocumentAbstraction <:< DocumentSerializer)
     ...
     *
     * For Cloudant, per the API doc: https://docs.cloudant.com/document.html
     ...
     * Cloudant generates one automatically as a UUID. If creation of the
    ...
    override protected[database] def put(d: DocumentAbstraction)(implicit transid: TransactionId): Future[DocInfo] = {
        reportFailure(
            Future {
                ...
                val response = if (!doc.update) db.save(doc) else db.update(doc)
     ...
     *
     * For Cloudant, per the API doc: https://docs.cloudant.com/document.html
     * If you fail to provide the latest _rev, Cloudant responds with a 409 error.
    ...
    override protected[database] def del(doc: DocInfo)(implicit transid: TransactionId): Future[Boolean] = {
        reportFailure(
            Future {
                ...
                val response = db.remove(doc.id(), doc.rev())
    ...
    override protected[core] def query(table: String, startKey: List[Any], endKey: List[Any], skip: Int, limit: Int, includeDocs: Boolean, descending: Boolean, reduce: Boolean)(
        ...
        reportFailure(
            Future {
                ...
                val limitedView = (if (limit > 0) view.limit(limit) else view).skip(skip)
                ...
                val result = keyedView.query(classOf[JsonObject])
     ...
     */
    def shutdown() = dbClient.shutdown()

    /** The service connector */
    private val dbClient = new Cloudant(dbUsername, dbPassword)

    /** The database connector */
    private val db = dbClient.getDb(dbName)

    /** Gson helper */
    private val gson = new GsonBuilder().create

    /** Describe cloudant response for logging */
    private def describe(r: Response) = if (r == null) "undefined" else s"${r.getId}[${r.getRev}] err=${r.getError} reason=${r.getReason}"

    /** Validates cloudant response */
    private def validate(response: Response) = {
