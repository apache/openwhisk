...
class RemoteCacheInvalidation(config: WhiskConfig, component: String, instance: InstanceId)(implicit ec: ExecutionContext, logging: Logging, as: ActorSystem) {
    ...
    private def notifyOtherInstancesAboutInvalidation(key: CacheKey): Future[Unit] = {
        cacheInvalidationProducer.send(topic, CacheInvalidationMessage(key, root)).map(_ => Unit)
    }

    val cache = WhiskEntityStore.cache(notifyOtherInstancesAboutInvalidation)
