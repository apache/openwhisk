 ...
 */
case class TransactionId private (meta: TransactionMetadata) extends Logging {
    def apply() = meta.id
     ...
     */
    def counting(from: AnyRef, marker: LogMarkerToken, message: String = "", logLevel: LogLevel = InfoLevel) = {
        emit(logLevel, this, from, message, Some(LogMarker(marker, deltaToStart)))
    }
     ...
     */
    def starting(from: AnyRef, marker: LogMarkerToken, message: String = "", logLevel: LogLevel = InfoLevel): StartMarker = {
        emit(logLevel, this, from, message, Some(LogMarker(marker, deltaToStart)))
     ...
     */
    def finished(from: AnyRef, startMarker: StartMarker, message: String = "", logLevel: LogLevel = InfoLevel, endTime: Instant = Instant.now(Clock.systemUTC)) = {
        val endMarker = LogMarkerToken(startMarker.startMarker.component, startMarker.startMarker.action, LoggingMarkers.finish)
        emit(logLevel, this, from, message, Some(LogMarker(endMarker, deltaToStart, Some(deltaToMarker(startMarker, endTime)))))
    }
    ...
    def failed(from: AnyRef, startMarker: StartMarker, message: String = "", logLevel: LogLevel = WarningLevel) = {
        ...
        emit(logLevel, this, from, message, Some(LogMarker(endMarker, deltaToStart, Some(deltaToMarker(startMarker)))))
    }
