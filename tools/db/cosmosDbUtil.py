...
DbContext = namedtuple(
    "DbContext", ["client", "db", "whisks", "subjects", "activations"]
)
verbose = False
...
def main():
    ...
    try:
        ...
        exit_code = {"init": init_cmd, "prune": prune_cmd, "drop": drop_cmd}[args.cmd](
            args, client
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--endpoint",
        help="DB Endpoint url like https://example.documents.azure.com:443/",
        required=True,
    )
    ...
    propmenu.add_argument(
        "db", help="Database name under which the collections would be created"
    )
    propmenu.add_argument("--dir", help="Directory under which auth files are stored")

    propmenu = subparsers.add_parser(
        "prune", help="remove stale databases created by test"
    )
    propmenu.add_argument(
        "--prefix",
        help="Database name prefix which are matched for removal",
        default="travis-",
    )
...
def find_default_subjects():
    ...
    for name in glob.glob1(files_dir, "auth.*"):
        ...
        doc = {
            "id": subject,
            "subject": subject,
            "namespaces": [{"name": subject, "uuid": uuid, "key": key}],
        }
        auth_file.close()
        yield doc
...
def init_coll(client, db, coll_name):
    query = client.QueryCollections(
        db["_self"], "SELECT * FROM root r WHERE r.id='" + coll_name + "'"
    )
    it = iter(query)
    coll = next(it, None)
    if coll is None:
        ...
        coll = client.CreateCollection(
            db["_self"], collection_definition, collection_options
        )
