...
node('xenial&&!H21&&!H22&&!H11&&!ubuntu-eu3') {
  ...
  stage("Build and Deploy to DockerHub") {
    ...
    withEnv(["Path+JDK=$JAVA_JDK_8/bin","JAVA_HOME=$JAVA_JDK_8"]) {
      ...
      def PUSH_CMD = "./gradlew distDocker -PdockerRegistry=docker.io -PdockerImagePrefix=openwhisk -x tests:dat:blackbox:badproxy:distDocker -x tests:dat:blackbox:badaction:distDocker -x sdk:docker:distDocker -x tools:cli:distDocker -x tools:cli:distDocker -x core:swift3Action:distDocker -x core:swift3.1.1Action:distDocker"
