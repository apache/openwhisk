...
node('xenial&&!H21&&!H22&&!H11&&!ubuntu-eu3') {
  ...
  stage("Build and Deploy to DockerHub") {
    ...
    withEnv(["Path+JDK=$JAVA_JDK_8/bin","JAVA_HOME=$JAVA_JDK_8"]) {
      ...
      def PUSH_CMD = "./gradlew :core:controller:distDocker :core:invoker:distDocker :core:monitoring:user-events:distDocker :tools:ow-utils:distDocker -PdockerRegistry=docker.io -PdockerImagePrefix=openwhisk"
